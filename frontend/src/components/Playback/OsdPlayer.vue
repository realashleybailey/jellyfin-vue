        ...
        <div class="d-flex align-center py-2 px-4">
          <div class="d-flex">
            ...
            <v-btn
              ...
              @click="videoPlayer.toggleMinimize" />
          </div>
          ...
          <div
            ...
            <div class="d-flex aligh-center ml-auto ml-md-0">
              ...
              <tooltip-button
                v-if="$vuetify.display.smAndUp"
                class="align-self-center active-button"
                :tooltip="{ text: $t('stretch'), location: 'top' }"
                :btn="{ icon: true }"
                @click="videoPlayer.isStretched = !videoPlayer.isStretched">
                <v-icon>
                  <i-mdi-stretch-to-page v-if="videoPlayer.isStretched" />
                  <i-mdi-stretch-to-page-outline v-else />
                </v-icon>
              </tooltip-button>
              <tooltip-button
                class="align-self-center active-button"
...
const keyPressMappings: { [key: string]: () => void } = {
  Spacebar: () => playbackManager.playPause(),
  ' ': () => playbackManager.playPause(),
  k: () => playbackManager.playPause(),
  ArrowRight: () => playbackManager.skipForward(),
  l: () => playbackManager.skipForward(),
  ArrowLeft: () => playbackManager.skipBackward(),
  j: () => playbackManager.skipBackward(),
  f: () => {
    if (playbackManager.currentlyPlayingMediaType === 'Video') {
      emit('toggleFullscreen');
    }
  },
  m: () => playbackManager.toggleMute()
};
...
function handleKeyPress(e: KeyboardEvent): void {
  if (keyPressMappings[e.key]) {
    keyPressMappings[e.key]();
  }
}
